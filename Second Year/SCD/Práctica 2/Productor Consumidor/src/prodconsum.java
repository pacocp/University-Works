...
{ private int numSlots = 0, cont = 0;   
  ...
  { enter(); 
    if(produciendo) producir.await();     //Si esta produciendo que espere        
    produciendo=true;   
    buffer[cont] = valor; cont++; 
    produciendo = false;
    consumir.signal();     //deja paso al consumir
    leave();  
  }
  ...
  { 
    enter();
    double valor;
    if(cont == 0) consumir.await(); //si el contador vale 0(no hay productos que consumir) que consumir espere
    cont--; valor = buffer[cont] ;
    producir.signal();  //deja paso a producir
    leave();
    return valor;
  }
}
