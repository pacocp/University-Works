...
_cubo cubo(5);
objeto_PLY ply("beethoven.ply");
objeto_PLY ply2("ant.ply");
objeto_PLY ply3("big_dodge.ply");
objeto_PLY ply4("perfil.ply");
objeto_PLY ply5("latacoc.ply");
revolucion_tex peon_tex(ply4.vertices, 40);
revolucion_tex coca(ply5.vertices, 40);
...
void draw_objects()
{
	...
	{
		cubo.draw_textura(1,tex2.imagen);
	}
	else if(pulso%13==3)
	{
		ply.draw_iluminacion_suave();
	}
	else if(pulso%13==4)
	{
		ply.draw_iluminacion_plana();
	}
	else if(pulso%13==5)
	{
		ply.draw_textura(1,tex2.imagen);
	}
	else if(pulso%13==6)
	{
		ply.draw_textura_ilum_suave(1,tex2.imagen);
	}
	else if(pulso%13==7)
	{
		ply.draw_textura_ilum_plana(1,tex2.imagen);
	}
	else if(pulso%13==8)
	{
		peon.draw_iluminacion_suave();
	}
	else if(pulso%13==9)
	{
		peon.draw_iluminacion_plana();
	}
	else if(pulso%13==10)
	{
		peon.draw_textura(1,tex.imagen);
	}
	else if(pulso%13==11)
	{
		peon_tex.draw_iluminacion_suave();
	}
	else if(pulso%13==12)
	{
		peon_tex.draw_iluminacion_plana();
	}
	else if(pulso%13==0)
	{
		peon_tex.draw_textura(1,tex.imagen);
	}
...
void luces()
{
	...
	GLfloat light_position2[4] = {0.0,0.0,10.0,1.0}; //al ser 0.0 es una luz en el infinito
	GLfloat color_luz_2[4] = {0.0,0.0,0.0,1.0};
	GLfloat color_luz_1[4] = {1.0,0.0,0.0,1.0};
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
